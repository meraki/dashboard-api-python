...
def build_working_configs(
    ...
    working_configs_with_feature = [config for config in working_configs if
                                    config[feature_old_toggle_param] != feature_old_toggle_disabled_mode]
...
def transform_coordinate(*, param, config, transform, **kwargs: dict):
    ...
    if transform['type'] == 'rename None':
        ...
        transformed_params, transformed_configs = transform_replace_none(param=param, config=config,
                                                                         transform=transform)
    ...
    if transform['type'] == 'demote dynamic key' and isinstance(config[param['names']['new']], dict):
        # Hand off the transform to a single-purpose method
        transformed_params, transformed_configs = transform_demote_dynamic_key(param=param, config=config,
                                                                               transform=transform,
    ...
    for task, config in zip(task_list, old_configs):
        ...
        for param in task:
            ...
            for transform in param['transforms']:
                transformed_params, transformed_configs = transform_coordinate(param=param, config=config,
                                                                               transform=transform,
                                                                               interface_ips=interface_ips)
                # Merge the modification sets from that method
                modified_params |= (transformed_params)
    ...
    for interface in old_configs:
        ...
        # Set the tagged VLAN info
            ...
            interface['interfaceIp'] = [tagged_vlan['interfaceIp'] for tagged_vlan \
                                        in settings['vlans']['others'] if
                                        tagged_vlan['id'] == interface['vlanId']].pop()
...
def configure_interface_dhcp(dashboard, serial, switch_interfaces_with_dhcp, created_interfaces):
    ...
    for interface in switch_interfaces_with_dhcp:
        created_dhcp_interface = [created_interface for created_interface in created_interfaces if
                                  created_interface['vlanId'] == interface['vlanId']].pop()
