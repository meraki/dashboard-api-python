...
def build_working_configs(
    ...
    working_configs_with_feature = [
        config
        for config in working_configs
        if config[feature_old_toggle_param] != feature_old_toggle_disabled_mode
    ]
...
def build_task_list(*, old_configs: list, mappings: list):
    ...
    for old_config in old_configs:
        ...
        old_config_matched_params = [
            param
            for param in mappings["knownParams"]
            if param["names"]["old"] in old_config
        ]
        ...
        remaining_params = [
            param
            for param in old_config_matched_params
            if param not in params_to["remove"]
        ]

        # REUSE
        params_to["reuse"] = [
            param for param in old_config_matched_params if param["status"] == "reused"
        ]
        remaining_params = [
            param for param in remaining_params if param not in params_to["reuse"]
        ]

        # RENAME
        params_to["rename"] = [
            param for param in old_config_matched_params if param["status"] == "renamed"
        ]
        remaining_params = [
            param for param in remaining_params if param not in params_to["rename"]
        ]
        ...
        params_to["transform"] = [
            param
            for param in old_config_matched_params
            if param["status"] == "transformed"
        ]
        remaining_params = [
            param for param in remaining_params if param not in params_to["transform"]
        ]

        if len(remaining_params) != 0:
            print(
                f"I found params in the source that aren't mapped in the mappings file. These are: {remaining_params}"
            )
...
def transform_split_mode(*, param, config, transform):
    ...
    config[param["names"]["new"]] = config[param["names"]["new"]].split(
        transform["action"]
    )
    modified_params.add(param["names"]["new"])
    modified_configs.add(config["name"])
...
def transform_coordinate(*, param, config, transform, **kwargs: dict):
    ...
    if transform["type"] == "rename None":
        ...
        transformed_params, transformed_configs = transform_replace_none(
            param=param, config=config, transform=transform
        )
    ...
    if transform["type"] == "rename mode":
        # Hand off the transform to a single-purpose method
        transformed_params, transformed_configs = transform_rename_mode(
    ...
    if (
        transform["type"] == "split delimited strings"
        and transform["action"] in config[param["names"]["new"]]
    ):
        # Hand off the transform to a single-purpose method
        transformed_params, transformed_configs = transform_split_mode(
            param=param, config=config, transform=transform
        )
        # Merge the modification sets from that method
        modified_params |= transformed_params
    ...
    if transform["type"] == "add param" and isinstance(
        config[param["names"]["new"]], list
    ):
        # Hand off the transform to a single-purpose method
        transformed_params, transformed_configs = transform_add_param(
            param=param, config=config, transform=transform
        )
        # Merge the modification sets from that method
        modified_params |= transformed_params
    ...
    if transform["type"] == "demote dynamic key" and isinstance(
        config[param["names"]["new"]], dict
    ):
        # Hand off the transform to a single-purpose method
        transformed_params, transformed_configs = transform_demote_dynamic_key(
            param=param, config=config, transform=transform, interface_ips=interface_ips
        )
        # Merge the modification sets from that method
        modified_params |= transformed_params
        modified_configs |= transformed_configs
    ...
    for task, config in zip(task_list, old_configs):
        ...
        for param in task:
            ...
            for transform in param["transforms"]:
                transformed_params, transformed_configs = transform_coordinate(
                    param=param,
                    config=config,
                    transform=transform,
    ...
    for interface in old_configs:
        ...
        # Set the tagged VLAN info
            ...
            interface["interfaceIp"] = [
                tagged_vlan["interfaceIp"]
                for tagged_vlan in settings["vlans"]["others"]
                if tagged_vlan["id"] == interface["vlanId"]
            ].pop()
...
def configure_interface_dhcp(
    dashboard, serial, switch_interfaces_with_dhcp, created_interfaces
):
    responses = []
    # Iterate through all the
    for interface in switch_interfaces_with_dhcp:
        created_dhcp_interface = [
            created_interface
            for created_interface in created_interfaces
            if created_interface["vlanId"] == interface["vlanId"]
        ].pop()
        if "dnsCustomNameservers" in interface:
            response = dashboard.switch.updateDeviceSwitchRoutingInterfaceDhcp(
                serial,
                created_dhcp_interface["interfaceId"],
...
def main():
    ...
    settings, mappings, tagged_vlan_ids, interface_ips = ingest(
        SETTINGS_FILENAME, MAPPINGS_FILENAME
    )
    ...
    switch_interfaces, switch_interfaces_with_dhcp = build_working_configs(
        appliance_vlans
    )
    ...
    # REMOVE params
    switch_interfaces = remove_params(task_list_types["remove"], switch_interfaces)

    # RENAME params
    switch_interfaces = rename_params(task_list_types["rename"], switch_interfaces)

    # TRANSFORM params. One of these transformations requires the list of interface IPs.
    switch_interfaces = transform_params(
        task_list_types["transform"], switch_interfaces, interface_ips=interface_ips
    )

    # ASSIGN statics (see settings.json)
    switch_interfaces = assign_statics(tagged_vlan_ids, settings, switch_interfaces)
    ...
    created_interfaces = create_interfaces(dashboard, settings, switch_interfaces)
