...
async def getNetworksLegacy(aiomeraki: meraki.aio.AsyncDashboardAPI, perPage=5):
    count = 0
    for x in await aiomeraki.organizations.getOrganizationNetworks(organizationId=ORGANIZATION_ID, perPage=perPage,
                                                                   total_pages=-1):
        print(f"{x['id']} - {x['name']}")
        count = count + 1
...
async def getNetworksIterator(aiomeraki: meraki.aio.AsyncDashboardAPI, perPage=5):
    count = 0
    async for x in aiomeraki.organizations.getOrganizationNetworks(organizationId=ORGANIZATION_ID, perPage=perPage,
                                                                   total_pages=-1):
        print(f"{x['id']} - {x['name']}")
        count = count + 1
    print(f"Found {count} networks")
...
async def getNetworkEventsLegacy(aiomeraki: meraki.aio.AsyncDashboardAPI, perPage=5):
    count = 0
    result = await aiomeraki.networks.getNetworkEvents(networkId=NETWORK_ID, perPage=perPage, total_pages=50,
...
async def getNetworkEventsIterator(aiomeraki: meraki.aio.AsyncDashboardAPI, perPage=5):
    count = 0
    async for x in aiomeraki.networks.getNetworkEvents(networkId=NETWORK_ID, perPage=perPage, total_pages=50,
                                                       productType="wireless"):
        print(f"{x['occurredAt']}")
        count = count + 1
    print(f"Found {count} events")
