...
def main(org_id, timespan):
    ...
    apiUsage = dashboard.organizations.getOrganizationApiRequests(
        org_id, timespan=timespan, total_pages=-1
    )
    csvString = (
        "method,host,path,queryString,tsDate,tsTime,responseCode,sourceIp,userAgent,"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate a CSV file of Meraki API activity for an organization."
    )
