...
async def analyzeOrganization(
    aiomeraki: meraki.aio.AsyncDashboardAPI, orgId: str, days: int
) -> Dict[str, int]:
    ret = {}
    timespan = days * 24 * 60 * 60
    events = await aiomeraki.appliance.getOrganizationApplianceSecurityEvents(
        orgId, timespan=timespan, total_pages=-1
    )
    for e in events:
        ip, port = e["srcIp"].rsplit(":", 1)
        ip = ip.strip("[]")  # remove brackets in case of ipv6
        if (
            not ipaddress.ip_address(ip).is_private
        ):  # don't block private ip addresses on the public ip of the firewall
            ret[ip] = ret.get(ip, 0) + 1

    return ret


async def updateFirewallrules(
    aiomeraki: meraki.aio.AsyncDashboardAPI, networkId: str, ip_list: List[str]
):
    rules = await aiomeraki.appliance.getNetworkApplianceFirewallL7FirewallRules(
        networkId
    )
    ...
    rules_to_add = [
        {"policy": "deny", "type": "ipRange", "value": x} for x in new_blocks
    ]
    ...
    await aiomeraki.appliance.updateNetworkApplianceFirewallL7FirewallRules(
        networkId, rules=rules
    )


async def main():
    parser = argparse.ArgumentParser(
        description="Block IP Addresses based on security events"
    )
    parser.add_argument(
        "-o",
        "--organization",
    ...
    async with meraki.aio.AsyncDashboardAPI(
        ...
        for x in organizations:
            if x["id"] in args.organizations or x["name"] in args.organizations:
                ...
                if args.save:
                    for n in await aiomeraki.organizations.getOrganizationNetworks(
                        x["id"]
                    ):
